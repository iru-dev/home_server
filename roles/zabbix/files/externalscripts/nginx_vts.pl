...
my $response = $ua->get('http://127.0.0.1:8095/stvhostKDqVDyPgGBc9eD/format/json');
...
foreach my $upstream_name (keys %{$json->{upstreamZones}}) {
  my $upstream = $json->{upstreamZones}{$upstream_name};
  foreach my $server (@$upstream) {
...
foreach my $u (@upstreams) {
  ...
  foreach my $k (keys %{$u->{data}}) {
    my $val = $u->{data}{$k};
    if (JSON::is_bool($val)) {
        print "- nginx_vts[$ukey,". lc($k). "] ". ($val eq 'true' ? 1 : 0). "\n";
    } elsif (ref $val) {
      foreach my $k2 (keys %$val) {
        my $val2 = $val->{$k2};
        print "- nginx_vts[$ukey,". lc($k.'_'.$k2). "] $val2\n";
      }
    } else {
      print "- nginx_vts[$ukey,". lc($k). "] $val\n";
      print "- nginx_vts[$ukey,responsesec] ". ($val/1000). "\n" if $k eq 'responseMsec';
    }
  }
}
